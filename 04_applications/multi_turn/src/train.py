...
import torch
from transformers import AutoTokenizer, GPT2LMHeadModel, get_polynomial_decay_schedule_with_warmup
...
class Trainer():
    ...
    def save(self):
        self.tokenizer.save_pretrained('/home/sslunder24/project/chatbot/koreanmulti/gpt2multi-de')
        self.model.save_pretrained('/home/sslunder24/project/chatbot/koreanmulti/gpt2multi-de')
...
if __name__=='__main__':
    ...
    assert args.model_type in [
        "gpt2", "skt/kogpt2-base-v2", "gpt2-large", "gpt2-xl",
        "openai-community/gpt2", "dbmdz/german-gpt2", "microsoft/DialoGPT-large"
    ], "Specify a correct GPT-2-based model type."
    
    args.data_dir = f"{args.data_dir}/{args.model_type}"
              
    trainer = Trainer(args)
    trainer.model.config.attn_pdrop = 0.2  # Dropout rate for attention probabilities
    trainer.model.config.resid_pdrop = 0.2  # Dropout rate for hidden states
    trainer.model.config.embd_pdrop = 0.2
    trainer.train()
    trainer.save()
